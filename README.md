...
# **텍스트 스타일 트랜스퍼 프로젝트 정리**

## **1. 프로젝트 개요**

한국어 텍스트를 대상으로 **데이터 증강 기반 스타일 변환(Text Style Transfer)** 모델을 구축하는 프로젝트.
기존 LLM에 LoRA 파인튜닝을 적용해 **원문 의미는 유지하면서 목표 스타일로 변환**하는 모델을 만드는 것이 목표이다.
...
증강된 두 스타일 문장 각각에 대해 임베딩을 추출한다.

* Sentence-BERT, KoSimCSE 등 한국어 임베딩 모델 활용
* **의미적으로 가까운 문장들끼리 매칭**
* 즉, “같은 의미 + 스타일만 다른 쌍”으로 pair 데이터셋 생성

> (선택) 문장 증강 단계에서 이미 의미 기반 샘플링을 적용해
> 의미 차이가 너무 큰 문장은 조합 과정에서 제거할 수도 있음.

---

## **2.4 LLM 파인튜닝 (LoRA)**

* 생성된 pair 데이터를 활용해 LLM을 **LoRA 방식으로 파인튜닝**
* 입력: 원문 + 목표 스타일 조건
* 출력: 목표 스타일로 변환된 문장
...
스타일 변환을 사용할 때 다음 조합을 활용:

* **파인튜닝된 LoRA**
* **few-shot 예시(prompt)**
  (few-shot 예시는 마찬가지로 데이터 증강 방식으로 확보 가능)

이 조합을 통해 **원문 의미 보존 + 목표 스타일 반영** 수행.

---

## **2.6 성능 평가**

스타일 트랜스퍼 품질 측정을 위해 다음 지표 활용:
